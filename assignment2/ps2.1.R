...
#c)
#The coefficient on women_parl indicates the predicted change in EPI score for a one-unit increase, a one-percent increase, of women in parliament. 

#To get the predicted difference between the 25th and 75th percentile, we can either multiply the coefficient by the IQR or use predict(). Both give the same result:
...
#b)
#The coefficient on women_parl decreases significantly once gov_eff is included. It might show that parts of the coefficient in the biavariate actually associated with the effect of government effectiveness. The gov_eff variable might be related to both women in parliament and environmental performance. 
...
#It measures the relationship between the omitted variable (gov_eff) and main X (women_parl); If women_parl goes up by 1 point, how much does gov_eff tend to go up?
...
#d) 
#The bias is positive because gov_eff is positively correlated with both women_parl and with the epi-score. 
...
#c) Conclusions won't be affected by this.  
